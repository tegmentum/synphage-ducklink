-- pipeline-native.sql
-- Pure-SQL synphage-ducklink pipeline for NATIVE DuckDB + ducklink-extension.
-- Runs in one `duckdb` invocation. No Python. No wasm CLI.
--
-- Assumes the ducklink extension has already been LOAD'd by the driver
-- (`LOAD '.../ducklink.duckdb_extension';`) with DUCKLINK_COMPONENTS pointing
-- at both blast.wasm and genome_format.wasm.

.print === setup ===

-- Read files into variables via read_text (native DuckDB, no extension).
SET VARIABLE gb_lambda = (SELECT content FROM read_text(getvariable('data_dir') || '/NC_001416.gb'));
SET VARIABLE gb_t7     = (SELECT content FROM read_text(getvariable('data_dir') || '/NC_001604.gb'));
SET VARIABLE gb_p22    = (SELECT content FROM read_text(getvariable('data_dir') || '/NC_002371.gb'));
...
-- Build queries JSON with native to_json + list + struct_pack — no jsonfns.
SET VARIABLE q_json = (
    SELECT to_json(list(struct_pack(
        key := concat(accession, ':', feature_index),
        data := sequence
    )))
    FROM gene_sample
);
...
-- Derive genome_features in the shape sql/best_hits.sql expects. json_extract_string
-- is part of DuckDB's built-in json extension (autoloaded).
